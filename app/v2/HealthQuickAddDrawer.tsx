...
type DraftSnapshot = { kind:Kind; date:string; time:string; sleepStart:string; sleepEnd:string; choice:string; name:string; quantity:number; duration:number; series:number }
...
export function HealthQuickAddDrawer({ state, today, commit, onClose }: Props) {
  ...
  const snapshot:DraftSnapshot={kind,date,time,sleepStart,sleepEnd,choice,name,quantity,duration,series}
  ...
  function persistDraft(next:DraftSnapshot){
    ...
    commit(current => {
      ...
      if(next.kind === 'sono'){
        ...
        day.sono = { ...(day.sono || {}), idLog:sessionId.current, deitar:next.sleepStart, acordar:next.sleepEnd, min:minutes, blocos:Math.floor(minutes/90), score, isDano:score<70, perigo:score<70?`Sono abaixo da meta (${score}%).`:'' }
      } else {
        ...
        const session = { idLog:sessionId.current, hora:next.time, isDano:false, perigo:'', itens:[entry] }

...
export function AppTopBar({ state, today, view, taskScope, commit, onTaskScopeChange }: Props) {
  ...
  const panel=<div className="mai-app-topbar-panel">
    ...
    {view==='tasks'?<><PanelSection title="Filtros"><SelectLine label="Prioridade" value={String(filter.priority||'all')} onChange={value=>patchFilter({priority:value})}>{priorityOptions}</SelectLine><SelectLine label="Data" value={String(filter.due||'all')} onChange={value=>patchFilter({due:value})}><option value="all">Todas</option><option value="overdue">Atrasadas</option><option value="today">Hoje</option><option value="future">Futuras</option><option value="none">Sem data</option></SelectLine></PanelSection><PanelSection title="Classificação"><Choices value={String(control.sort||'manual')} onChange={value=>patchControl({sort:value})} items={[{value:'manual',label:'Manual'},{value:'date',label:'Data'},{value:'priority',label:'Prioridade'},{value:'name',label:'Nome'}]}/></PanelSection><PanelSection title="Visualização"><Choices value={taskScope.startsWith('project:')?'project':taskScope} onChange={value=>{if(value==='project'){const first=projects[0];if(first)onTaskScopeChange(`project:${String(first.id)}`);return}onTaskScopeChange(value as TaskModuleScope)}} items={[{value:'entrada',label:'Entrada'},{value:'today',label:'Hoje'},{value:'upcoming',label:'Em breve'},{value:'project',label:'Projeto'}]}/>{projects.length?<SelectLine label="Projeto atual" value={taskScope.startsWith('project:')?taskScope.slice(8):''} onChange={value=>value&&onTaskScopeChange(`project:${value}`)}><option value="">Selecionar</option>{projects.map(project=><option key={String(project.id)} value={String(project.id)}>{String(project.nome||'Projeto')}</option>)}</SelectLine>:null}</PanelSection></>:null}

    {view==='habits'?<><PanelSection title="Filtros"><SelectLine label="Situação" value={String(filter.status||'all')} onChange={value=>patchFilter({status:value})}><option value="all">Todos</option><option value="pending">Pendentes</option><option value="done">Realizados</option></SelectLine></PanelSection><PanelSection title="Classificação"><Choices value={String(control.sort||'name')} onChange={value=>patchControl({sort:value})} items={[{value:'name',label:'Nome'},{value:'time',label:'Horário'},{value:'progress',label:'Progresso'}]}/></PanelSection><PanelSection title="Visualização"><Choices value={areaTabs.habits||'today'} onChange={value=>setAreaTab('habits',value)} items={[{value:'today',label:'Hoje'},{value:'week',label:'Semana'},{value:'report',label:'Relatório'}]}/><Choices value={advanced.habits?'advanced':'simple'} onChange={value=>setAdvanced('habits',value==='advanced')} items={[{value:'simple',label:'Simples'},{value:'advanced',label:'Avançada'}]}/></PanelSection></>:null}

    {view==='goals'?<><PanelSection title="Filtros"><SelectLine label="Situação" value={String(filter.status||'all')} onChange={value=>patchFilter({status:value})}><option value="all">Em andamento + pausadas</option><option value="active">Em andamento</option><option value="paused">Pausadas</option></SelectLine><SelectLine label="Prioridade" value={String(filter.priority||'all')} onChange={value=>patchFilter({priority:value})}>{priorityOptions}</SelectLine></PanelSection><PanelSection title="Classificação"><Choices value={String(control.sort||'date')} onChange={value=>patchControl({sort:value})} items={[{value:'date',label:'Prazo'},{value:'priority',label:'Prioridade'},{value:'progress',label:'Progresso'},{value:'name',label:'Nome'}]}/></PanelSection><PanelSection title="Visualização"><Choices value={advanced.goals?'advanced':'simple'} onChange={value=>setAdvanced('goals',value==='advanced')} items={[{value:'simple',label:'Simples'},{value:'advanced',label:'Avançada'}]}/><Choices value={density} onChange={value=>patchControl({density:value})} items={[{value:'comfortable',label:'Padrão'},{value:'compact',label:'Compacta'}]}/></PanelSection></>:null}

    {view==='notes'?<><PanelSection title="Filtros"><SelectLine label="Notas" value={String(filter.pinned||'all')} onChange={value=>patchFilter({pinned:value})}><option value="all">Todas</option><option value="pinned">Fixadas</option><option value="unpinned">Não fixadas</option></SelectLine></PanelSection><PanelSection title="Classificação"><Choices value={String(control.sort||'recent')} onChange={value=>patchControl({sort:value})} items={[{value:'recent',label:'Recentes'},{value:'oldest',label:'Antigas'},{value:'name',label:'Título'}]}/></PanelSection><PanelSection title="Visualização"><Choices value={advanced.notes?'advanced':'simple'} onChange={value=>setAdvanced('notes',value==='advanced')} items={[{value:'simple',label:'Simples'},{value:'advanced',label:'Avançada'}]}/><Choices value={density} onChange={value=>patchControl({density:value})} items={[{value:'comfortable',label:'Padrão'},{value:'compact',label:'Compacta'}]}/></PanelSection></>:null}

    {view==='finance'?<><PanelSection title="Filtros"><SelectLine label="Tipo" value={String(filter.type||'all')} onChange={value=>patchFilter({type:value})}><option value="all">Todos</option><option value="expense">Despesas</option><option value="income">Receitas</option></SelectLine><SelectLine label="Status" value={String(filter.status||'all')} onChange={value=>patchFilter({status:value})}><option value="all">Todos</option><option value="pending">Pendentes</option><option value="paid">Pagos</option></SelectLine></PanelSection><PanelSection title="Classificação"><Choices value={String(control.sort||'date')} onChange={value=>patchControl({sort:value})} items={[{value:'date',label:'Data'},{value:'value',label:'Valor'},{value:'name',label:'Nome'}]}/></PanelSection><PanelSection title="Visualização"><Choices value={areaTabs.finance||'overview'} onChange={value=>setAreaTab('finance',value)} items={[{value:'overview',label:'Resumo'},{value:'transactions',label:'Lançamentos'},{value:'accounts',label:'Contas'},{value:'cards',label:'Cartões'}]}/><Choices value={advanced.finance?'advanced':'simple'} onChange={value=>setAdvanced('finance',value==='advanced')} items={[{value:'simple',label:'Simples'},{value:'advanced',label:'Avançada'}]}/></PanelSection></>:null}

    {view==='health'?<><PanelSection title="Filtros"><SelectLine label="Período" value={String(filter.period||'30')} onChange={value=>patchFilter({period:value})}><option value="7">7 dias</option><option value="30">30 dias</option><option value="all">Todo histórico</option></SelectLine></PanelSection><PanelSection title="Classificação"><Choices value={String(control.sort||'recent')} onChange={value=>patchControl({sort:value})} items={[{value:'recent',label:'Recentes'},{value:'oldest',label:'Antigos'}]}/></PanelSection><PanelSection title="Visualização"><Choices value={areaTabs.health||'today'} onChange={value=>setAreaTab('health',value)} items={[{value:'today',label:'Hoje'},{value:'history',label:'Histórico'}]}/><Choices value={density} onChange={value=>patchControl({density:value})} items={[{value:'comfortable',label:'Padrão'},{value:'compact',label:'Compacta'}]}/></PanelSection></>:null}

    {view==='files'?<><PanelSection title="Filtros"><SelectLine label="Tipo" value={String(filter.kind||'all')} onChange={value=>patchFilter({kind:value})}><option value="all">Todos</option><option value="folder">Pastas</option><option value="file">Arquivos</option></SelectLine></PanelSection><PanelSection title="Classificação"><Choices value={String(control.sort||'name')} onChange={value=>patchControl({sort:value})} items={[{value:'name',label:'Nome'},{value:'recent',label:'Recentes'}]}/></PanelSection><PanelSection title="Visualização"><Choices value={areaTabs.files||'list'} onChange={value=>setAreaTab('files',value)} items={[{value:'list',label:'Lista'},{value:'grid',label:'Grade'}]}/><Choices value={density} onChange={value=>patchControl({density:value})} items={[{value:'comfortable',label:'Padrão'},{value:'compact',label:'Compacta'}]}/></PanelSection></>:null}
  </div>

  return <div className="mai-app-topbar"><div className="mai-app-topbar-context"><strong>{title}</strong><span>{info}</span></div><div className="mai-app-topbar-single-action"><button className="mai-app-topbar-menu-button" data-active={open} title="Opções do módulo" aria-label="Opções do módulo" onClick={()=>setOpen(value=>!value)}><span className="material-symbols-rounded">tune</span><span>Opções</span></button></div>{open?panel:null}</div>
}

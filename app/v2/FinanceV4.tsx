...
export function FinanceV4({ state, today, commit, createRequest, inspect: _inspect }: { state: MaiState; today: string; commit: Commit; createRequest?: string; inspect: (item: InspectableItem) => void }) {
  ...
  return <div className="mai-v3-area-page mai-v4-finance">
    ...
    {tab === 'overview' ? <><div className="mai-finance-v4-overview-metrics">
      <article className="mai-finance-v4-metric-primary" data-negative={realBalance < 0}><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">account_balance_wallet</span><span>Saldo atual</span></div><strong>{money.format(realBalance)}</strong><small>Disponível agora nas suas contas</small></article>
      <article className="mai-finance-v4-metric-primary" data-negative={projectedBalance < 0}><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">timeline</span><span>Previsto</span></div><strong>{money.format(projectedBalance)}</strong><small>Acumulado até {monthLabel(month)}</small></article>
      ...
      <article className="mai-finance-v4-metric" data-negative={monthExpense > 0}><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">north_east</span><span>Despesas</span></div><strong>{money.format(monthExpense)}</strong><small>{monthLabel(month)}</small></article>
      <article className="mai-finance-v4-metric" data-positive={monthResult > 0} data-negative={monthResult < 0}><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">balance</span><span>Balanço</span></div><strong>{money.format(monthResult)}</strong><small>{monthResult >= 0 ? 'Superávit do mês' : 'Déficit do mês'}</small></article>
      <article className="mai-finance-v4-metric"><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">credit_card</span><span>Cartões</span></div><strong>{money.format(monthInvoice)}</strong><small>{cards.length} {cards.length === 1 ? 'cartão' : 'cartões'} · faturas do mês</small></article>
      <button type="button" className="mai-finance-v4-metric-asset" onClick={() => setTab('boxes')}><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">savings</span><span>Caixinhas</span></div><strong>{money.format(totalBoxes)}</strong><div className="mai-finance-v4-metric-caption"><span>{boxes.length} {boxes.length === 1 ? 'caixinha' : 'caixinhas'}{totalBoxTarget ? ` · meta ${money.format(totalBoxTarget)}` : ''}</span><strong>{totalBoxTarget ? `${Math.round(totalBoxProgress)}%` : ''}</strong></div>{totalBoxTarget ? <div className="mai-finance-v4-metric-progress"><i><b style={{ width: `${totalBoxProgress}%` }} /></i><small>Progresso total das suas metas</small></div> : <small>Dinheiro separado para seus objetivos</small>}</button>
      <button type="button" className="mai-finance-v4-metric-asset" data-positive={investmentResult > 0} data-negative={investmentResult < 0} onClick={() => setTab('investments')}><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">trending_up</span><span>Investimentos</span></div><strong>{money.format(totalInvestmentValue)}</strong><div className="mai-finance-v4-metric-caption"><span>Aportado {money.format(totalInvested)}</span><strong data-positive={investmentResult >= 0} data-negative={investmentResult < 0}>{investmentResult >= 0 ? '+' : ''}{money.format(investmentResult)}{totalInvested ? ` · ${investmentResultPercent.toFixed(1)}%` : ''}</strong></div><small>Valor atual da sua carteira</small></button>

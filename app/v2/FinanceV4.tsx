...
const financeIcons = MAI_ENTITY_ICONS
...
function cleanPayments(value: unknown, fallbackDate = '') {
  return rows(value).map(payment => ({
    ...payment,
    id: String(payment.id || uid('pay')),
    data: dateKey(payment.data) || fallbackDate,
    valor: clampMoney(payment.valor),
  })).filter(payment => payment.valor > 0)
}

function paymentState(item: Row, value: unknown = item.pagamentos) {
  const total = clampMoney(item.valor)
  const payments = cleanPayments(value, dateKey(item.data))
  if (payments.length) {
    const paid = Math.min(total, payments.reduce((sum, payment) => sum + clampMoney(payment.valor), 0))
    return { payments, paid, remaining: Math.max(0, total - paid), status: total > 0 && paid >= total ? 'pago' : paid > 0 ? 'parcial' : 'pendente' }
  }
  const legacyPaid = Math.min(total, clampMoney(item.valor_pago) || (item.status === 'pago' ? total : 0))
  return { payments, paid: legacyPaid, remaining: Math.max(0, total - legacyPaid), status: total > 0 && legacyPaid >= total ? 'pago' : legacyPaid > 0 ? 'parcial' : 'pendente' }
}
...
export function FinanceV4({ state, today, commit, createRequest, inspect: _inspect }: { state: MaiState; today: string; commit: Commit; createRequest?: string; inspect: (item: InspectableItem) => void }) {
  ...
  const paidAmount = (item: Row) => paymentState(item).paid
  const paidAmountThrough = (item: Row, cutoffDate: string) => {
    ...
    return Math.min(clampMoney(item.valor), clampMoney(item.valor_pago) || (item.status === 'pago' ? clampMoney(item.valor) : 0))
  }
  ...
  function fixedRuleAtMonth(rule: Row, monthKey: string): Row | null {
    ...
    const value = occurrence?.valor_override === '' || occurrence?.valor_override == null ? clampMoney(rule.valor) : clampMoney(occurrence.valor_override)
    return { ...rule, ...occurrence, id: rule.id, _isFixed: true, occurrenceKey: `${rule.id}|${monthKey}`, data: occurrence?.data_override || `${monthKey}-${String(day).padStart(2, '0')}`, valor: value, status, valor_pago: occurrence?.valor_pago ?? rule.valor_pago ?? (status === 'pago' ? value : 0), pagamentos: cleanPayments(occurrence?.pagamentos, occurrence?.data_override || `${monthKey}-${String(day).padStart(2, '0')}`), recorrencia: 'fixo' } as Row
  }
    ...
    commit(current => {
      ...
      if (kind === 'transaction') {
        ...
        if (recurring === 'fixo') {
          ...
          const occurrencePayments = cleanPayments(clean.pagamentos, dateKey(clean.data) || today)
          const value = clampMoney(clean.valor)
          const paid = occurrencePayments.length ? Math.min(value, occurrencePayments.reduce((sum, payment) => sum + clampMoney(payment.valor), 0)) : Math.min(value, clampMoney(clean.valor_pago) || (clean.status === 'pago' ? value : 0))
          const occurrenceStatus = value > 0 && paid >= value ? 'pago' : paid > 0 ? 'parcial' : 'pendente'
          ...
          const occurrence = { chave: occurrenceKey, fixo_id: fixedRule.id, competencia: month, status: occurrenceStatus, valor_pago: paid, pagamentos: occurrencePayments, atualizado_em: new Date().toISOString() }
  ...
  function togglePaid(item: Row) {
    const total = clampMoney(item.valor)
    const current = paymentState(item)
    if (item.status === 'pago') {
      if (item._isFixed) { updateOccurrence(item, { status: 'pendente', valor_pago: 0, pagamentos: [] }); return }
      commit(stateNow => ({ ...stateNow, finance: { ...stateNow.finance, transactions: rows(stateNow.finance.transactions).map(row => String(row.id) === String(item.id) ? { ...row, status: 'pendente', valor_pago: 0, pagamentos: [] } : row) } }))
      return
    }
    if (!total) return
    const payments = current.remaining > 0 ? [...current.payments, { id: uid('pay'), data: today, valor: current.remaining }] : current.payments
    if (item._isFixed) { updateOccurrence(item, { status: 'pago', valor_pago: total, pagamentos: payments }); return }
  ...
  function setDraftPaymentStatus(value: string) {
    if (!draft || kind !== 'transaction') return
    if (value === 'pendente') {
      setDraft({ ...draft, status: 'pendente', valor_pago: 0, pagamentos: [] })
      return
    }
  ...
  function editFixedOccurrence(item = draft) {
    ...
    if (!Number.isFinite(parsedValue)) { alert('Digite um valor válido.'); return }
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || Number.isNaN(new Date(`${date}T12:00:00`).getTime())) { alert('Digite uma data válida no formato AAAA-MM-DD.'); return }
    const currentPayments = cleanPayments(currentOccurrence.pagamentos, date)
    const paid = Math.min(parsedValue, currentPayments.reduce((sum, payment) => sum + clampMoney(payment.valor), 0))
    updateOccurrence({ ...item, occurrenceKey: `${item.id}|${month}` }, { valor_override: parsedValue, data_override: date, pagamentos: currentPayments, valor_pago: paid, status: parsedValue > 0 && paid >= parsedValue ? 'pago' : paid > 0 ? 'parcial' : 'pendente' })
  }
  ...
  function addPayment() {
    ...
    const answer = prompt(`Valor do pagamento (restante ${money.format(current.remaining)}):`, current.remaining.toFixed(2).replace('.', ',')); if (answer === null) return
    const amount = parseMoneyInput(answer)
    if (!Number.isFinite(amount) || amount <= 0) { alert('Digite um valor de pagamento válido.'); return }
    if (amount - current.remaining > 0.005) { alert(`O pagamento não pode ser maior que o restante de ${money.format(current.remaining)}.`); return }
    const paymentDate = prompt('Data do pagamento (AAAA-MM-DD):', today); if (paymentDate === null) return
    if (!/^\d{4}-\d{2}-\d{2}$/.test(paymentDate) || Number.isNaN(new Date(`${paymentDate}T12:00:00`).getTime())) { alert('Digite uma data válida no formato AAAA-MM-DD.'); return }
    const payments = [...current.payments, { id: uid('pay'), data: paymentDate, valor: amount, criado_em: new Date().toISOString() }]
    const next = paymentState({ ...draft, pagamentos: payments, valor_pago: 0, status: 'pendente' }, payments)
    setDraft({ ...draft, pagamentos: next.payments, valor_pago: next.paid, status: next.status })
  }
  ...
  function generateInstallments() {
    ...
    const generated = Array.from({ length: quantity }, (_, index) => ({ ...draft, id: index === 0 ? String(draft.id || uid('fin')) : uid('fin'), lote_id: lot, parcela_numero: index + 1, parcelas_total: quantity, titulo: `${baseTitle} (${index + 1}/${quantity})`, valor: installment, data: moveDateForInstallment(dateKey(draft.data) || today, index, String(draft.intervalo_parcelas || 'mensal')), status: index === 0 ? paymentState({ ...draft, valor: installment }).status : 'pendente', valor_pago: index === 0 ? Math.min(installment, paidAmount(draft)) : 0, pagamentos: index === 0 ? cleanPayments(draft.pagamentos, dateKey(draft.data)) : [], recorrencia: 'unico', parcelas: undefined, intervalo_parcelas: undefined, dividir_total: undefined, dia_mes: undefined, mes_inicio: undefined, mes_fim: undefined, ativo: undefined, _persisted: undefined, _isFixed: undefined }))
  ...
  function adjustCardInvoice(card: Row) {
    ...
    commit(current => {
      ...
      const existingPayments = cleanPayments(existing?.pagamentos, dueDate)
      const paid = existing ? Math.min(value, existingPayments.length ? existingPayments.reduce((sum, payment) => sum + clampMoney(payment.valor), 0) : paidAmount(existing)) : 0
      const status = value > 0 && paid >= value ? 'pago' : paid > 0 ? 'parcial' : 'pendente'
      const next: Row = {
        ...(existing || {}),
  ...
  function moveBoxValue(direction: 'in' | 'out') {
    ...
    const value = parseMoneyInput(answer); if (!Number.isFinite(value) || !value) return
  ...
  const draftPayment = draft && kind === 'transaction' ? paymentState(draft) : null
  ...
  return <div className="mai-v3-area-page mai-v4-finance">
    ...
    <div className="mai-v3-area-tabs mai-finance-v4-tabs">{tabs.map(item => <button key={item.id} data-active={tab === item.id} onClick={() => setTab(item.id)}>{item.icon ? <MaiIcon name={item.icon} size={18} /> : null}<span>{item.label}</span></button>)}</div>

    {tab === 'overview' ? <><div className="mai-finance-v4-overview-metrics">
      <article className="mai-finance-v4-metric" data-negative={realBalance < 0}><div className="mai-finance-v4-metric-label"><MaiIcon name="account_balance_wallet" size={19} /><span>Saldo atual</span></div><strong>{money.format(realBalance)}</strong><small>Disponível agora</small></article>
      <article className="mai-finance-v4-metric" data-negative={projectedBalance < 0}><div className="mai-finance-v4-metric-label"><MaiIcon name="timeline" size={19} /><span>Previsto</span></div><strong>{money.format(projectedBalance)}</strong><small>Até {monthLabel(month)}</small></article>
      <article className="mai-finance-v4-metric" data-positive={monthIncome > 0}><div className="mai-finance-v4-metric-label"><MaiIcon name="south_west" size={19} /><span>Receitas</span></div><strong>{money.format(monthIncome)}</strong><small>{monthLabel(month)}</small></article>
      <article className="mai-finance-v4-metric" data-negative={monthExpense > 0}><div className="mai-finance-v4-metric-label"><MaiIcon name="north_east" size={19} /><span>Despesas</span></div><strong>{money.format(monthExpense)}</strong><small>Inclui faturas de cartão</small></article>
      <article className="mai-finance-v4-metric" data-positive={monthResult > 0} data-negative={monthResult < 0}><div className="mai-finance-v4-metric-label"><MaiIcon name="balance" size={19} /><span>Balanço</span></div><strong>{money.format(monthResult)}</strong><small>{monthResult >= 0 ? 'Superávit' : 'Déficit'}</small></article>
      <button type="button" className="mai-finance-v4-metric mai-finance-v4-metric-asset" onClick={() => setTab('boxes')}><div className="mai-finance-v4-metric-label"><MaiIcon name="savings" size={19} /><span>Caixinhas</span></div><strong>{money.format(totalBoxes)}</strong><small>{boxes.length} {boxes.length === 1 ? 'caixinha' : 'caixinhas'}{totalBoxTarget ? ` · ${Math.round(totalBoxProgress)}%` : ''}</small></button>
      <button type="button" className="mai-finance-v4-metric mai-finance-v4-metric-asset" data-positive={investmentResult > 0} data-negative={investmentResult < 0} onClick={() => setTab('investments')}><div className="mai-finance-v4-metric-label"><MaiIcon name="trending_up" size={19} /><span>Investimentos</span></div><strong>{money.format(totalInvestmentValue)}</strong><small>{investmentResult >= 0 ? '+' : ''}{money.format(investmentResult)}{totalInvested ? ` · ${investmentResultPercent.toFixed(1)}%` : ''}</small></button>
    </div><section className="mai-v3-simple-section mai-finance-v4-overview-list"><div className="mai-finance-v4-section-head"><div><h2>{filtersActive ? 'Lançamentos filtrados' : 'Últimos lançamentos'}</h2><small>{filtered.length} em {monthLabel(month)}</small></div><button type="button" onClick={() => openNew('transaction')}><MaiIcon name="add" size={18} />Adicionar lançamento</button></div><div className="mai-v3-finance-rows mai-v3-simple-list">{recent.map(transactionRow)}{!recent.length ? <div className="mai-v3-empty-line">Nenhum lançamento encontrado.</div> : null}</div></section></> : null}

    {tab === 'transactions' ? <section className="mai-v3-simple-section"><div className="mai-finance-v4-section-head"><div><h2>Lançamentos</h2><small>{filtered.length} no período · únicos, parcelados e fixos</small></div><button type="button" onClick={() => openNew('transaction')}><MaiIcon name="add" size={18} />Adicionar lançamento</button></div><div className="mai-v3-finance-rows mai-v3-simple-list">{filtered.map(transactionRow)}{!filtered.length ? <div className="mai-v3-empty-line">Nenhum lançamento encontrado.</div> : null}</div></section> : null}
    {tab === 'accounts' ? <section className="mai-v3-simple-section"><div className="mai-finance-v4-section-head"><div><h2>Contas</h2><small>Saldos reais e conciliação.</small></div><button type="button" onClick={() => openNew('account')}><MaiIcon name="add" size={18} />Adicionar conta</button></div><div className="mai-v3-account-list mai-finance-v4-entity-list mai-finance-v4-account-list">{visibleAccounts.map(account => <article key={String(account.id)} className="mai-finance-v4-entity-row"><FinanceEntityIcon photo={String(account.foto || '')} icon="account_balance" color={String(account.cor || '#718269')} /><button type="button" className="mai-finance-v4-entity-identity" onClick={() => openExisting('account', account)}><strong>{account.nome}</strong><small>Saldo inicial {money.format(Number(account.saldo_inicial || 0))}</small></button><span className="mai-finance-v4-entity-summary"><b>{money.format(accountBalance(account))}</b><small>saldo atual</small></span><button type="button" className="mai-finance-v4-row-action" onClick={() => reconcile(account)}><MaiIcon name="sync_alt" size={18} /><span>Conciliar</span></button></article>)}</div></section> : null}
    {tab === 'cards' ? <section className="mai-v3-simple-section"><div className="mai-finance-v4-section-head"><div><h2>Cartões</h2><small>Fatura, limite disponível e vencimento.</small></div><button type="button" onClick={() => openNew('card')}><MaiIcon name="add" size={18} />Adicionar cartão</button></div><div className="mai-v3-card-list mai-finance-v4-card-list mai-finance-v4-entity-list">{visibleCards.map(card => { const invoice = cardInvoice(card, monthItems); return <article key={String(card.id)} className="mai-finance-v4-entity-row mai-finance-v4-card-row"><FinanceEntityIcon photo={String(card.foto || '')} icon="credit_card" color={String(card.cor || '#8a7d72')} /><button type="button" className="mai-finance-v4-entity-identity" onClick={() => openExisting('card', card)}><strong>{card.nome}</strong><small>Fecha dia {card.fechamento || '—'} · vence dia {card.vencimento || '—'}</small></button><span className="mai-finance-v4-entity-summary"><b>{money.format(invoice)}</b><small>fatura · {money.format(Math.max(0, clampMoney(card.limite) - invoice))} disponível</small></span><button type="button" className="mai-finance-v4-row-action" onClick={() => adjustCardInvoice(card)}><MaiIcon name="edit_note" size={18} /><span>Ajustar fatura</span></button></article> })}</div></section> : null}

    {tab === 'boxes' ? <section className="mai-v3-simple-section mai-finance-v4-goal-area"><div className="mai-finance-v4-section-head"><div><h2>Caixinha</h2><small>{money.format(totalBoxes)} guardados em {boxes.length} {boxes.length === 1 ? 'meta' : 'metas'}.</small></div><button type="button" onClick={() => openNew('box')}><MaiIcon name="add" size={18} />Nova caixinha</button></div><div className="mai-finance-v4-goal-grid">{boxes.map(box => { const target = clampMoney(box.meta); const current = clampMoney(box.saldo); const progress = target ? Math.min(100, current / target * 100) : 0; return <article key={String(box.id)} className="mai-finance-v4-goal-card" style={{ '--finance-item-color': box.cor || '#718269' } as React.CSSProperties}><button type="button" className="mai-finance-v4-goal-main" onClick={() => openExisting('box', box)}><FinanceEntityIcon photo={String(box.foto || '')} icon={String(box.icone || 'savings')} color={String(box.cor || '#718269')} className="mai-finance-v4-goal-icon" /><span className="mai-finance-v4-goal-copy"><strong>{box.nome}</strong><small>{box.prazo ? `Meta até ${naturalDate(dateKey(box.prazo), today)}` : 'Sem prazo definido'}</small></span><b>{money.format(current)}</b></button><div className="mai-finance-v4-progress"><i><b style={{ width: `${progress}%` }} /></i><small>{target ? `${Math.round(progress)}% de ${money.format(target)}` : 'Defina um valor-alvo'}</small></div><button type="button" className="mai-finance-v4-quick-action" onClick={() => { openExisting('box', box); setTimeout(() => { const el = document.querySelector('.mai-finance-v4-drawer-box .mai-finance-v4-positive-action') as HTMLButtonElement | null; el?.focus() }, 0) }}>Adicionar valor</button></article> })}{!boxes.length ? <div className="mai-finance-v4-empty-feature"><MaiIcon name="savings" size={28} /><strong>Crie sua primeira caixinha</strong><small>Carro, casa, viagem, reserva ou qualquer objetivo que você queira construir aos poucos.</small><button type="button" onClick={() => openNew('box')}>Criar caixinha</button></div> : null}</div></section> : null}

    {tab === 'investments' ? <section className="mai-v3-simple-section mai-finance-v4-goal-area"><div className="mai-finance-v4-section-head"><div><h2>Investimentos</h2><small>{money.format(totalInvestmentValue)} atuais · {money.format(totalInvested)} aportados.</small></div><button type="button" onClick={() => openNew('investment')}><MaiIcon name="add" size={18} />Novo investimento</button></div><div className="mai-finance-v4-investment-summary"><span>Patrimônio investido<strong>{money.format(totalInvestmentValue)}</strong></span><span>Total aportado<strong>{money.format(totalInvested)}</strong></span><span>Resultado<strong data-positive={totalInvestmentValue - totalInvested >= 0}>{money.format(totalInvestmentValue - totalInvested)}</strong></span></div><div className="mai-finance-v4-goal-grid">{investments.map(item => { const gain = clampMoney(item.valor_atual) - clampMoney(item.aportado); const percent = clampMoney(item.aportado) ? gain / clampMoney(item.aportado) * 100 : 0; return <article key={String(item.id)} className="mai-finance-v4-goal-card" style={{ '--finance-item-color': item.cor || '#607d6b' } as React.CSSProperties}><button type="button" className="mai-finance-v4-goal-main" onClick={() => openExisting('investment', item)}><FinanceEntityIcon photo={String(item.foto || '')} icon={String(item.icone || 'trending_up')} color={String(item.cor || '#607d6b')} className="mai-finance-v4-goal-icon" /><span className="mai-finance-v4-goal-copy"><strong>{item.nome}</strong><small>{item.tipo || 'Investimento'}{item.instituicao ? ` · ${item.instituicao}` : ''}</small></span><b>{money.format(clampMoney(item.valor_atual))}</b></button><div className="mai-finance-v4-investment-result"><span>Aportado {money.format(clampMoney(item.aportado))}</span><strong data-positive={gain >= 0} data-negative={gain < 0}>{gain >= 0 ? '+' : ''}{money.format(gain)} · {percent.toFixed(1)}%</strong></div></article> })}{!investments.length ? <div className="mai-finance-v4-empty-feature"><MaiIcon name="trending_up" size={28} /><strong>Comece sua carteira de longo prazo</strong><small>Acompanhe aportes, valor atual e evolução dos seus investimentos.</small><button type="button" onClick={() => openNew('investment')}>Adicionar investimento</button></div> : null}</div></section> : null}

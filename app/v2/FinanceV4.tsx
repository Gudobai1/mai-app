...
export function FinanceV4({ state, today, commit, createRequest, inspect: _inspect }: { state: MaiState; today: string; commit: Commit; createRequest?: string; inspect: (item: InspectableItem) => void }) {
  ...
  function newDraft(type: DraftKind): Row {
    ...
    if (type === 'box') return { id: uid('box'), nome: '', icone: 'savings', cor: '#718269', meta: 0, saldo: 0, prazo: '', observacao: '', movimentos: [], _persisted: false }
    return { id: uid('inv'), nome: '', icone: 'trending_up', cor: '#607d6b', tipo: 'ETF', instituicao: '', aportado: 0, valor_atual: 0, observacao: '', movimentos: [], _persisted: false }
  }
  ...
  const showMonth = !['boxes', 'investments'].includes(tab)
  ...
  return <div className="mai-v3-area-page mai-v4-finance">
    ...
    {tab === 'overview' ? <><div className="mai-finance-v4-overview-metrics"><article className="mai-finance-v4-metric" data-negative={displayRealBalance < 0}><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">account_balance_wallet</span><span>Saldo Atual</span></div><strong>{money.format(displayRealBalance)}</strong><small>{filtersActive ? 'Resultado realizado do filtro' : 'Saldo consolidado agora'}</small></article><article className="mai-finance-v4-metric" data-negative={displayProjectedBalance < 0}><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">trending_up</span><span>Previsto</span></div><strong>{money.format(displayProjectedBalance)}</strong><small>{filtersActive ? 'Resultado previsto do filtro' : 'Saldo consolidado projetado'}</small></article><article className="mai-finance-v4-metric" data-positive={filteredIncome > 0}><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">south_west</span><span>Receitas</span></div><strong>{money.format(filteredIncome)}</strong><small>{monthLabel(month)}</small></article><article className="mai-finance-v4-metric" data-negative={filteredExpense > 0}><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">north_east</span><span>Despesas</span></div><strong>{money.format(filteredExpense)}</strong><small>{monthLabel(month)}</small></article><article className="mai-finance-v4-metric"><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">credit_card</span><span>Cartões</span></div><strong>{money.format(filteredInvoice)}</strong><small>Faturas do mês</small></article><article className="mai-finance-v4-metric" data-positive={filteredResult > 0} data-negative={filteredResult < 0}><div className="mai-finance-v4-metric-label"><span className="material-symbols-rounded">balance</span><span>Balanço Mensal</span></div><strong>{money.format(filteredResult)}</strong><small>Receitas − despesas</small></article></div><section className="mai-v3-simple-section mai-finance-v4-overview-list"><div className="mai-finance-v4-section-head"><div><h2>{filtersActive ? 'Lançamentos filtrados' : 'Últimos lançamentos'}</h2><small>{filtered.length} em {monthLabel(month)}</small></div><button type="button" onClick={() => openNew('transaction')}><span className="material-symbols-rounded">add</span>Adicionar lançamento</button></div><div className="mai-v3-finance-rows mai-v3-simple-list">{recent.map(transactionRow)}{!recent.length ? <div className="mai-v3-empty-line">Nenhum lançamento encontrado.</div> : null}</div></section></> : null}

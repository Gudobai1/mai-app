...
export function FinanceV4({state,today,commit,createRequest,inspect}:{state:MaiState;today:string;commit:Commit;createRequest?:string;inspect:(item:InspectableItem)=>void}){
  ...
  const row=(item:Row)=>{const incomeItem=item.tipo==='receita';const status=String(item.status||'').trim();return <button key={String(item.id)} className="mai-today-unified-row mai-item-row-v2 mai-v4-finance-item" onClick={()=>open(item)}><i className="mai-today-unified-dot" style={{borderColor:incomeItem?'var(--mai-success, #5d8a68)':'var(--mai-danger, #c85b52)'}}/><span className="mai-item-copy-v2"><span className="mai-item-titleline-v2"><strong>{String(item.titulo||item.descricao||'Lançamento')}</strong><span className="mai-item-inline-tag">{String(item.categoria||'Sem categoria')}</span></span><span className="mai-item-subline-v2"><span>{naturalDate(dateKey(item.data),today)}</span><span>·</span><span>{incomeItem?'+':'−'} {money.format(Number(item.valor||0))}{status?` · ${status}`:''}</span></span></span></button>}

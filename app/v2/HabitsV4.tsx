...
export function HabitsV4({ state, today, commit, createRequest }: { state:MaiState; today:string; commit:Commit; createRequest?:string }) {
  ...
  return <div className="mai-v3-area-page mai-v4-habits">
    ...
    {tab==='today'?<section className="mai-v3-simple-section"><h2>Hoje</h2><div className="mai-v3-simple-list">{todayHabits.map(habit=>{const value=current(habit,today);const complete=done(habit,today);return <article className="mai-v3-habit-today" key={String(habit.id)}><button className="mai-v3-habit-check" data-done={complete} onClick={()=>mark(habit,today)}>{complete?'✓':''}</button><button className="mai-v3-habit-body" onClick={()=>setDraft({...habit,dias_semana:values(habit.dias_semana)})}><strong>{habit.nome}</strong>{quantitative(habit)?<small>{value} de {target(habit)} {habit.unidade||''}</small>:habit.hora?<small>{habit.hora}</small>:null}</button>{quantitative(habit)?<button className="mai-v4-habit-value" onClick={()=>mark(habit,today)}>{value||0}</button>:null}</article>})}{!todayHabits.length?<div className="mai-v3-empty-line">Nenhum hábito previsto para hoje.</div>:null}</div></section>:null}

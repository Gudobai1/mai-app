...
export function HabitsV4({ state, today, commit, createRequest, inspect }: { state:MaiState; today:string; commit:Commit; createRequest?:string; inspect:(item:InspectableItem)=>void }) {
  ...
  useEffect(()=>{if(createRequest?.startsWith('habits:')){setDraft({nome:'',descricao:'',meta:1,unidade:'',hora:'',cor_hex:'var(--v3-accent)',icone:'star',dias_semana:[0,1,2,3,4,5,6],ativo:true});setCreateTool('')}},[createRequest])
  ...
  function save(e:FormEvent){e.preventDefault();if(!draft||!String(draft.nome||'').trim())return;const next={...draft,id:draft.id||uid('hab'),nome:String(draft.nome).trim(),meta:Math.max(1,Number(draft.meta||1)),dias_semana:values(draft.dias_semana).map(Number),ativo:true};commit(currentState=>({...currentState,habits:rows(currentState.habits).some(item=>String(item.id)===String(next.id))?rows(currentState.habits).map(item=>String(item.id)===String(next.id)?next:item):[...rows(currentState.habits),next]}));setDraft(null);setCreateTool('')}
  ...
  const createDays=draft?values(draft.dias_semana).map(Number):[]
  const daysSummary=createDays.length===7?'Todos os dias':createDays.join(',')==='1,2,3,4,5'?'Dias úteis':createDays.length?`${createDays.length} dias por semana`:'Não selecionado'
  ...
  return <div className="mai-v3-area-page mai-v4-habits">
    ...
    {draft?<div className="mai-v3-create-layer" onMouseDown={()=>setDraft(null)}><form className="mai-v3-create-drawer" onSubmit={save} onMouseDown={e=>e.stopPropagation()}><header className="mai-v3-drawer-header"><strong>Novo hábito</strong><button type="button" className="mai-v3-close" onClick={()=>setDraft(null)}>×</button></header><div className="mai-v3-drawer-body mai-create-unified-body" onMouseDown={()=>createTool&&setCreateTool('')}><input className="mai-v3-title-input" autoFocus value={draft.nome||''} placeholder="Nome do hábito" onMouseDown={e=>e.stopPropagation()} onChange={e=>setDraft({...draft,nome:e.target.value})}/><textarea className="mai-v3-description-input mai-create-unified-description" rows={2} value={draft.descricao||''} placeholder="Descrição" onMouseDown={e=>e.stopPropagation()} onChange={e=>setDraft({...draft,descricao:e.target.value})}/><div className="mai-task-v4-toolbar mai-context-unified-tools mai-create-unified-tools" onMouseDown={e=>e.stopPropagation()}>
      ...
      <CreateTool id="habit-days" icon="date_range" label="Dias" summary={daysSummary} color="#4f7cac" open={createTool} setOpen={setCreateTool}><div className="mai-v3-day-picker">{['D','S','T','Q','Q','S','S'].map((label,day)=>{const selected=createDays.includes(day);return <button type="button" key={`${label}-${day}`} data-active={selected} onClick={()=>setDraft({...draft,dias_semana:selected?createDays.filter(v=>v!==day):[...createDays,day].sort()})}>{label}</button>})}</div></CreateTool>

...
function RichNoteEditor({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  function command(name: string, argument?: string) { ref.current?.focus(); document.execCommand(name, false, argument); onChange(ref.current?.innerHTML || '') }
...
export function NotesV4({ state, commit, createRequest, googleRpc = defaultGoogleRpc }: { state: MaiState; today: string; commit: Commit; createRequest?: string; inspect: (item: InspectableItem) => void; googleRpc?: Rpc }) {
  ...
  useEffect(() => { if (createRequest?.startsWith('notes:')) { setTab('active'); setDraft(newNote()) } }, [createRequest])
  ...
  function normalized(note: Row): Row {
    ...
    return { ...clean, id: clean.id || uid('note'), titulo: String(clean.titulo || '').trim(), data: new Date().toISOString(), ativo: clean.ativo !== false, arquivado: clean.arquivado === true, fixado: clean.fixado === true, tamanho: ['normal', 'largo', 'grande'].includes(String(clean.tamanho)) ? clean.tamanho : 'normal', anexos: rows(clean.anexos), ordem: Number(clean.ordem || Date.now()) }
  }
  ...
  function quickAction(note: Row, kind: NoteAction) {
    ...
    commit(current => ({ ...current, notes: kind === 'delete' ? rows(current.notes).filter(item => String(item.id) !== String(note.id)) : rows(current.notes).map(item => String(item.id) === String(note.id) ? { ...item, fixado: kind === 'pin' ? !item.fixado : item.fixado, arquivado: kind === 'archive' ? true : kind === 'restore' ? false : item.arquivado, ativo: kind === 'trash' ? false : kind === 'restore' ? true : item.ativo, data: new Date().toISOString() } : item) }))
    if (draft && String(draft.id) === String(note.id)) setDraft(null)
  }

  function draftAction(kind: Exclude<NoteAction, 'pin'>) {
    if (!draft?.id) return
    if (kind === 'delete') { if (!confirm('Excluir definitivamente esta nota e seus vínculos?')) return; commit(current => ({ ...current, notes: rows(current.notes).filter(item => String(item.id) !== String(draft.id)) })); setDraft(null); return }
  ...
  function reorder(sourceId: string, targetId: string) {
    ...
    const ordered = [...visible]; const from = ordered.findIndex(item => String(item.id) === sourceId); const to = ordered.findIndex(item => String(item.id) === targetId)
    if (from < 0 || to < 0) return
    const [moved] = ordered.splice(from, 1); ordered.splice(to, 0, moved); const rank = new Map(ordered.map((item, index) => [String(item.id), index]))
  ...
  async function upload(file?: File) {
    ...
    try {
      ...
      const response = await googleRpc('salvarAnexoDrive', [data, file.name, file.type]); const item = response?.item || response
      setDraft(current => current ? { ...current, anexos: [...rows(current.anexos), { idDrive: item.id, nome: item.name || item.nome || file.name, tipo: item.tipo || item.mimeType || file.type, mimeType: item.mimeType || item.tipo || file.type, url: item.url || item.webViewLink || '', webViewLink: item.webViewLink || item.url || '' }] } : current)
  ...
  return <>
    <div className="mai-notes-v4">
      ...
      <div className="mai-notes-v4-controls"><div className="mai-notes-v4-tabs" role="tablist" aria-label="Visualização das notas"><button data-active={tab === 'active'} onClick={() => changeTab('active')}>Notas</button><button data-active={tab === 'archive'} onClick={() => changeTab('archive')}>Arquivo</button><button data-active={tab === 'trash'} onClick={() => changeTab('trash')}>Lixeira</button></div><label className="mai-notes-v4-search"><span className="material-symbols-rounded">search</span><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Buscar nas notas" />{query ? <button type="button" aria-label="Limpar busca" onClick={() => setQuery('')}><span className="material-symbols-rounded">close</span></button> : null}</label></div>

      <div className="mai-v3-note-list mai-notes-v4-list" data-tab={tab}>{visible.map(note => { const attachments = rows(note.anexos).length; return <article className="mai-notes-v4-row mai-item-row-v2" key={String(note.id)} draggable={tab === 'active'} onDragStart={event => event.dataTransfer.setData('text/note', String(note.id))} onDragOver={event => { if (tab === 'active') event.preventDefault() }} onDrop={event => { event.preventDefault(); reorder(event.dataTransfer.getData('text/note'), String(note.id)) }} onClick={() => setDraft({ ...note, anexos: rows(note.anexos), _persisted: true })}><span className="mai-notes-v4-row-icon" aria-hidden="true"><span className="material-symbols-rounded">notes</span></span><span className="mai-item-copy-v2 mai-notes-v4-row-copy"><span className="mai-item-titleline-v2"><strong>{note.titulo || 'Sem título'}</strong></span><span className="mai-item-subline-v2 mai-notes-v4-row-context"><span>{contextLabel(note)}</span><span>·</span><span>{formatDate(note.data)}</span>{attachments ? <><span>·</span><span className="mai-notes-v4-attachment-context"><span className="material-symbols-rounded">attach_file</span>{attachments} {attachments === 1 ? 'anexo' : 'anexos'}</span></> : null}</span></span>{tab !== 'trash' ? <button type="button" className="mai-notes-v4-row-action" title={note.fixado ? 'Desfixar' : 'Fixar'} aria-label={note.fixado ? 'Desfixar nota' : 'Fixar nota'} onClick={event => { event.stopPropagation(); quickAction(note, 'pin') }}><span className="material-symbols-rounded">{note.fixado ? 'keep_off' : 'keep'}</span></button> : null}</article> })}</div>

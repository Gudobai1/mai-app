...
export function TodayV4({state,today,commit,navigate,inspect,onSearch}:{state:MaiState;today:string;commit:(change:(current:MaiState)=>MaiState)=>void;navigate:(view:AppView)=>void;inspect:(item:InspectableItem)=>void;onSearch:()=>void;onMore:()=>void}){
  ...
  const sectionContent:Record<SectionId,React.ReactNode>={
    ...
    habits:<section className="mai-v4-today-list-section"><header><h2>Hábitos</h2></header><div className="mai-today-unified-list">{habits.map(habit=>{const value=Number(entries.find(entry=>String(entry.habito_id)===String(habit.id))?.valor||0);const target=Math.max(1,Number(habit.meta||1));const done=value>=target;const detail=target>1||habit.unidade?`${value} de ${target} ${String(habit.unidade||'')}`:done?'Concluído':'Pendente';return <button className="mai-today-unified-row mai-item-row-v2" key={String(habit.id)} onClick={()=>inspect({kind:'habit',sourceId:String(habit.id),title:String(habit.nome||'Hábito'),date:today,raw:habit})}><i className="mai-today-unified-dot" data-done={done} style={!done?{borderColor:String(habit.cor_hex||'var(--v3-accent)')}:undefined}>{done?'✓':''}</i><span className="mai-item-copy-v2"><span className="mai-item-titleline-v2"><strong>{String(habit.nome||'Hábito')}</strong></span><span className="mai-item-subline-v2"><span>Hoje</span><span>·</span><span>{detail}</span></span></span></button>})}{!habits.length?<div className="mai-v3-empty-line">Nenhum hábito previsto para hoje.</div>:null}</div></section>,
    goals:<section className="mai-v4-today-list-section"><header><h2>Metas</h2></header><div className="mai-today-unified-list">{goals.map(goal=>{const total=Math.max(1,Number(goal.progresso_total||100));const pct=Math.max(0,Math.min(100,Math.round(Number(goal.progresso_atual||0)/total*100)));return <button className="mai-today-unified-row mai-item-row-v2" key={String(goal.id)} onClick={()=>inspect({kind:'goal',sourceId:String(goal.id),title:String(goal.titulo||'Meta'),date:goalDate(goal),raw:goal})}><i className="mai-today-unified-dot" style={{borderColor:priorityColor(goal.prioridade)}}/><span className="mai-item-copy-v2"><span className="mai-item-titleline-v2"><strong>{String(goal.titulo||'Meta')}</strong></span><span className="mai-item-subline-v2"><span>Hoje</span><span>·</span><span>{pct}%</span></span></span></button>})}{!goals.length?<div className="mai-v3-empty-line">Nenhuma meta com prazo para hoje.</div>:null}</div></section>,
    finance:<section className="mai-v4-today-list-section"><header><h2>Finanças</h2></header><div className="mai-today-unified-list">{transactions.map(item=>{const income=String(item.tipo||'').toLowerCase()==='receita';return <button className="mai-today-unified-row mai-item-row-v2" key={String(item.id)} onClick={()=>inspect({kind:'finance',sourceId:String(item.id),title:txTitle(item),date:dateKey(item.data),raw:item})}><i className="mai-today-unified-dot" style={{borderColor:income?'var(--mai-success, #5d8a68)':'var(--mai-danger, #c85b52)'}}/><span className="mai-item-copy-v2"><span className="mai-item-titleline-v2"><strong>{txTitle(item)}</strong></span><span className="mai-item-subline-v2"><span>Hoje</span><span>·</span><span>{income?'+':'−'} {money.format(Number(item.valor||0))}</span></span></span></button>})}{!transactions.length?<div className="mai-v3-empty-line">Nenhum lançamento para hoje.</div>:null}</div></section>,

...
export function TodayV4({state,today,commit,navigate,inspect,onSearch}:{state:MaiState;today:string;commit:(change:(current:MaiState)=>MaiState)=>void;navigate:(view:AppView)=>void;inspect:(item:InspectableItem)=>void;onSearch:()=>void;onMore:()=>void}){
  ...
  const goals=rows(state.goals).filter(goal=>goalDate(goal)===today&&!String(goal.status||'').toLocaleLowerCase('pt-BR').includes('conclu')&&goal.concluida!==true)
  const finance=state.finance||{}
  const transactions=rows(finance.transactions).filter(item=>dateKey(item.data)===today&&!item.ignorar_calculo&&!paid(item.status))

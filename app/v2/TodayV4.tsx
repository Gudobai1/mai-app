...
export function TodayV4({state,today,commit,navigate,inspect,onSearch}:{state:MaiState;today:string;commit:(change:(current:MaiState)=>MaiState)=>void;navigate:(view:AppView)=>void;inspect:(item:InspectableItem)=>void;onSearch:()=>void;onMore:()=>void}){
  ...
  const sectionContent:Record<SectionId,React.ReactNode>={
    ...
    finance:<section className="mai-v4-today-list-section"><header><h2>Finanças</h2></header><div className="mai-today-unified-list">{transactions.map(item=>{const income=String(item.tipo||'').toLowerCase()==='receita';const day=dateKey(item.data);const payment=financePayment(item);const partial=payment.paid>0&&payment.remaining>0;const displayValue=partial?payment.remaining:payment.total;return <article className="mai-today-unified-row mai-item-row-v2 mai-v4-today-finance-row" key={String(item.id)}><button className="mai-v4-today-finance-main" type="button" onClick={()=>inspect({kind:'finance',sourceId:String(item.id),title:txTitle(item),date:day,raw:item})}><i className="mai-today-unified-dot" style={{borderColor:income?'var(--mai-success, #5d8a68)':'var(--mai-danger, #c85b52)'}}/><span className="mai-item-copy-v2"><span className="mai-item-titleline-v2"><strong>{txTitle(item)}</strong></span><span className="mai-item-subline-v2"><span>{naturalDate(day,today)}</span><span>·</span><span>{income?'+':'−'} {money.format(displayValue)}</span>{partial?<><span>·</span><span>pago {money.format(payment.paid)}</span></>:null}</span></span></button><button type="button" className="mai-v4-today-finance-hide" title="Ocultar da tela inicial" aria-label={`Ocultar ${txTitle(item)} da tela inicial`} onClick={()=>setFinanceVisible(item,false)}><span className="material-symbols-rounded">visibility_off</span></button></article>})}{!transactions.length?<div className="mai-v3-empty-line">Nenhum lançamento pendente para hoje.</div>:null}</div></section>,
  ...
  return <div className={`mai-v4-today-single${kanban?' mai-v4-today-kanban':''}`}><div className="mai-v4-today-header-wrap"><TodayCompact part="header" state={state} today={today} commit={commit} navigate={navigate} inspect={inspect} onSearch={onSearch} onMore={()=>setMenuOpen(v=>!v)}/>{menuOpen?<div className="mai-v4-today-customize"><strong>Personalizar Hoje</strong>{order.map((id,index)=><div key={id}><button className="mai-v4-section-toggle" data-visible={!hidden.has(id)} onClick={()=>setHidden(id)}><span className="material-symbols-rounded">{hidden.has(id)?'check_box_outline_blank':'check_box'}</span>{labels[id]}</button><span><button disabled={index===0} onClick={()=>move(id,-1)} title="Mover para cima">↑</button><button disabled={index===order.length-1} onClick={()=>move(id,1)} title="Mover para baixo">↓</button></span></div>)}{financeCandidates.length?<><strong>Lançamentos na tela inicial</strong>{financeCandidates.map(item=>{const visible=item.ocultar_inicio!==true&&item.ocultar_home!==true;return <div key={`finance-visibility-${String(item.id)}`}><button className="mai-v4-section-toggle" onClick={()=>setFinanceVisible(item,!visible)}><span className="material-symbols-rounded">{visible?'visibility':'visibility_off'}</span><span className="mai-v4-today-customize-label">{txTitle(item)}</span></button></div>})}</>:null}</div>:null}</div><div className={`mai-v4-today-list${kanban?' mai-native-kanban-board':''}`}>{order.filter(id=>!hidden.has(id)).map(id=><div key={id}>{sectionContent[id]}</div>)}</div></div>
}

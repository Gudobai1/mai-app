...
export function TodayV4({state,today,commit,navigate,inspect,onSearch}:{state:MaiState;today:string;commit:(change:(current:MaiState)=>MaiState)=>void;navigate:(view:AppView)=>void;inspect:(item:InspectableItem)=>void;onSearch:()=>void;onMore:()=>void}){
  ...
  const sectionContent:Record<SectionId,React.ReactNode>={
    ...
    finance:<section className="mai-v4-today-list-section"><header><h2>Finanças</h2></header><div className="mai-today-unified-list mai-v3-finance-rows mai-v3-simple-list">{transactions.map(item=>{const day=dateKey(item.data);return <FinanceTransactionCard key={String(item.id)} item={item} accounts={financeAccounts} cards={financeCards} today={today} onOpen={()=>inspect({kind:'finance',sourceId:String(item.id),title:txTitle(item),date:day,raw:item})} onTogglePaid={()=>toggleFinancePaid(item)}/>})}{!transactions.length?<div className="mai-v3-empty-line">{financeCandidates.length?'Nenhum lançamento visível na tela inicial.':'Nenhum lançamento pendente para hoje.'}</div>:null}</div></section>,

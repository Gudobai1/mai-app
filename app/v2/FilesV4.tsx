...
function bytes(value: unknown) {
  const size = Number(value || 0); if (!size) return '—'
  const units = ['B', 'KB', 'MB', 'GB', 'TB']; let n = size, i = 0
  while (n >= 1024 && i < units.length - 1) { n /= 1024; i += 1 }
  return `${n >= 10 || i === 0 ? n.toFixed(0) : n.toFixed(1)} ${units[i]}`
}
function dateLabel(value: unknown) {
  const raw = String(value || ''); if (!raw) return '—'
  const d = new Date(raw); return Number.isNaN(d.getTime()) ? '—' : d.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })
}
...
export function FilesV4({ state, commit, createRequest }: Props) {
  ...
  const computerFolders = rows(state.configs.filesComputerFolders).map(folder => ({ ...folder, id: String(folder.id || folder.folderId || ''), name: String(folder.name || folder.nome || 'Pasta do computador'), nome: String(folder.name || folder.nome || 'Pasta do computador'), mimeType: FOLDER, tipo: 'folder', computerShortcut: true })).filter(folder => folder.id)
  const canWrite = view === 'meudrive' || view === 'drive' || (view === 'computers' && folderId !== 'root')
  ...
  async function load() {
    ...
    try {
      if (view === 'computers' && folderId === 'root') {
        setItems(computerFolders); setPath([]); setSelected(new Set()); return
      }
      const apiView = view === 'computers' ? 'meudrive' : view
      const response = await driveCall('list', [folderId, apiView, driveId])
      setItems(rows(response?.items)); setPath(rows(response?.path)); setSelected(new Set())
    } catch (loadError: any) { setError(loadError?.message || 'Não foi possível acessar os arquivos.') }
  ...
  useEffect(() => { if (!query.trim()) void load() }, [view, driveId, folderId, state.configs.filesComputerFolders])
  useEffect(() => {
  ...
  function openFolder(item: Row) {
    if (view === 'computers') { setFolderId(String(item.id)); setQuery(''); setSelected(new Set()); return }
  ...
  function openItem(item: Row) { if (isFolder(item)) openFolder(item); else setPreview(item) }
  ...
  async function run(action: () => Promise<void>) { setBusy(true); setError(''); try { await action() } catch (actionError: any) { setError(actionError?.message || 'Não foi possível concluir a ação.') } finally { setBusy(false) } }
  ...
  async function makeFolder() {
    const name = folderName.trim(); if (!name || !canWrite) return
    await run(async () => { await driveCall('mkdir', [name, currentParent || 'root']); setNewFolder(false); setFolderName('Nova pasta'); await load() })
  }
  async function rename(item = one) { if (!item || item.computerShortcut) return; const name = window.prompt('Novo nome:', String(item.name || item.nome || '')); if (!name?.trim()) return; await run(async () => { await driveCall('rename', [item.id, name.trim()]); await load() }) }
  async function star(itemsToUse = selectedItems) { const editable = itemsToUse.filter(item => !item.computerShortcut); if (!editable.length) return; const next = !editable.every(item => item.starred === true); await run(async () => { for (const item of editable) await driveCall('star', [item.id, next]); await load() }) }
  function copy(mode: 'copy' | 'cut', itemsToUse = selectedItems) { const editable = itemsToUse.filter(item => !item.computerShortcut); if (!editable.length) return; setClipboard({ mode, items: editable }); setContext(null) }
  async function paste() { if (!clipboard?.items.length || !canWrite) return; const destination = currentParent || 'root'; await run(async () => { if (clipboard.mode === 'copy') await driveCall('copy', [clipboard.items.map(item => item.id), destination]); else { for (const item of clipboard.items) await driveCall('move', [item.id, destination]); setClipboard(null) } await load() }) }
  async function trash(itemsToUse = selectedItems) { const editable = itemsToUse.filter(item => !item.computerShortcut); if (!editable.length) return; await run(async () => { await driveCall('trash', [editable.map(item => item.id)]); await load() }) }
  ...
  async function loadPicker(folder: string, pickerDriveId: string, pickerView: 'meudrive' | 'drive') { const response = await driveCall('list', [folder, pickerView, pickerDriveId]); setPickerFolders(rows(response?.items).filter(isFolder)); setPickerPath(rows(response?.path)) }
  function openMove(itemsToUse = selectedItems) { const editable = itemsToUse.filter(item => !item.computerShortcut); if (!editable.length) return; const next = { ids: editable.map(item => String(item.id)), folder: 'root', driveId: '', view: 'meudrive' as const }; setMoveDialog(next); void loadPicker('root', '', 'meudrive') }
  function pickerLocation(folder: string, pickerDriveId: string, pickerView: 'meudrive' | 'drive') { setMoveDialog(current => current ? { ...current, folder, driveId: pickerDriveId, view: pickerView } : current); void loadPicker(folder, pickerDriveId, pickerView) }
  ...
  function startDrag(item: Row) { if (item.computerShortcut) return; const ids = selected.has(String(item.id)) ? [...selected] : [String(item.id)]; dragIds.current = ids; if (!selected.has(String(item.id))) setSelected(new Set(ids)) }
  function dropOnFolder(event: any, item: Row) { event.preventDefault(); event.stopPropagation(); if (!isFolder(item)) return; if (event.dataTransfer?.files?.length) { void uploadFiles(event.dataTransfer.files, String(item.id)); return } void moveDropped(dragIds.current, String(item.id)) }
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (mod && event.key.toLowerCase() === 'v' && clipboard && canWrite) { event.preventDefault(); void paste() }
  ...
  }, [visibleItems, selectedItems, clipboard, view, one, currentParent, canWrite, preview])
  ...
  const breadcrumbVisible = !query.trim() && (view === 'meudrive' || view === 'drive' || view === 'computers')

  return <>
    <div className="mai-drive-v4" onClick={() => setContext(null)} onDragOver={event => { if (event.dataTransfer.types.includes('Files') && canWrite) event.preventDefault() }} onDrop={event => { if (event.dataTransfer.files?.length && canWrite) { event.preventDefault(); void uploadFiles(event.dataTransfer.files, currentParent || 'root') } }}>
      <aside className="mai-drive-v4-sidebar">
        <button className="mai-drive-new" disabled={!canWrite} onClick={() => canWrite && setNewFolder(true)}><span className="material-symbols-rounded">add</span><span>Novo</span></button>
        <nav>
      ...
      <main className="mai-drive-v4-main">
        <header className="mai-drive-v4-head">
          <div className="mai-drive-title"><h1>{title}</h1>{view === 'computers' ? <p>Pastas principais dos backups dos seus computadores.</p> : null}</div>
        ...
        {breadcrumbVisible ? <div className="mai-drive-breadcrumbs">
          <button onClick={() => { setFolderId('root'); setPath([]) }}>{view === 'drive' ? (sharedDrives.find(d => String(d.id) === driveId)?.name || 'Drive') : view === 'computers' ? 'Computadores' : 'Meu Drive'}</button>
        ...
        <section className={`mai-drive-content mai-drive-${layout}`} onClick={event => { if (event.currentTarget === event.target) clearSelection() }}>
          ...
          {loading ? <div className="mai-drive-empty">Carregando arquivos…</div> : visibleItems.map(item => <article key={String(item.id)} draggable={view !== 'trash' && !item.computerShortcut} data-selected={selected.has(String(item.id))} data-cut={clipboard?.mode === 'cut' && clipboard.items.some(clip => String(clip.id) === String(item.id))} onDragStart={() => startDrag(item)} onDragOver={event => { if (isFolder(item)) event.preventDefault() }} onDrop={event => dropOnFolder(event, item)} onClick={event => { event.stopPropagation(); clickItem(event, item) }} onDoubleClick={() => openItem(item)} onContextMenu={event => { event.preventDefault(); event.stopPropagation(); if (!selected.has(String(item.id))) setSelected(new Set([String(item.id)])); setContext({ x: event.clientX, y: event.clientY, item }) }}>
            <span className="mai-drive-file-name"><i><span className="material-symbols-rounded">{fileIcon(item)}</span></i><span><strong>{String(item.name || item.nome || 'Arquivo')}</strong>{layout === 'grid' ? <small>{item.computerShortcut ? 'Backup do computador' : isFolder(item) ? 'Pasta' : dateLabel(item.modifiedTime || item.modificado)}</small> : null}</span>{item.starred ? <span className="material-symbols-rounded mai-drive-star">star</span> : null}{item.shared ? <span className="material-symbols-rounded mai-drive-shared">group</span> : null}</span>
            {layout === 'list' ? <><span>{item.computerShortcut ? 'Backup' : String(item.owners?.[0]?.displayName || item.owners?.[0]?.emailAddress || (item.shared ? 'Compartilhado' : 'Eu'))}</span><span>{item.computerShortcut ? '—' : dateLabel(item.modifiedTime || item.modificado)}</span><span>{isFolder(item) ? '—' : bytes(item.size || item.tamanho)}</span><button aria-label="Mais ações" onClick={event => { event.stopPropagation(); setContext({ x: event.clientX, y: event.clientY, item }) }}><span className="material-symbols-rounded">more_vert</span></button></> : null}
          </article>)}
          {!loading && !visibleItems.length ? <div className="mai-drive-empty"><span className="material-symbols-rounded">folder_open</span><strong>{query.trim() ? 'Nenhum resultado' : view === 'computers' && folderId === 'root' ? 'Nenhuma pasta de computador cadastrada' : 'Nenhum arquivo aqui'}</strong><small>{query.trim() ? 'Tente outro termo de pesquisa.' : view === 'computers' && folderId === 'root' ? 'As pastas principais dos seus backups aparecerão aqui.' : canWrite ? 'Arraste arquivos para cá ou use “Enviar arquivo”.' : 'Esta área está vazia.'}</small></div> : null}
        </section>
      </main>
      ...
      {context ? <div className="mai-drive-context" style={{ left: Math.min(context.x, window.innerWidth - 230), top: Math.min(context.y, window.innerHeight - 360) }} onClick={event => event.stopPropagation()}>
        <button onClick={() => { openItem(context.item); setContext(null) }}><span className="material-symbols-rounded">visibility</span>{isFolder(context.item) ? 'Abrir pasta' : 'Visualizar'}</button>
        {!context.item.computerShortcut ? <button onClick={() => void rename(context.item)}><span className="material-symbols-rounded">edit</span>Renomear</button> : null}
        {!context.item.computerShortcut ? <button onClick={() => void star([context.item])}><span className="material-symbols-rounded">star</span>{context.item.starred ? 'Remover estrela' : 'Adicionar estrela'}</button> : null}
        {!context.item.computerShortcut ? <><hr/><button onClick={() => copy('cut', [context.item])}><span className="material-symbols-rounded">content_cut</span>Recortar</button><button onClick={() => copy('copy', [context.item])}><span className="material-symbols-rounded">content_copy</span>Copiar</button><button onClick={() => openMove([context.item])}><span className="material-symbols-rounded">drive_file_move</span>Mover para</button><hr/>{view === 'trash' ? <><button onClick={() => void restore([context.item])}><span className="material-symbols-rounded">restore_from_trash</span>Restaurar</button><button className="danger" onClick={() => void deleteForever([context.item])}><span className="material-symbols-rounded">delete_forever</span>Excluir definitivamente</button></> : <button className="danger" onClick={() => void trash([context.item])}><span className="material-symbols-rounded">delete</span>Mover para lixeira</button>}</> : null}
      </div> : null}

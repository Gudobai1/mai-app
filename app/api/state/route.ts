...
export async function GET(request:Request){const auth=await scopedClient(request);if(!('client'in auth)||!auth.client||!auth.user)return Response.json({error:auth.error},{status:auth.status});const client=auth.client;const user=auth.user;const{data,error}=await client.from('mai_state').select('data,updated_at').eq('user_id',user.id).maybeSingle();if(error)return Response.json({error:error.message},{status:500});let state=data?.data??null;try{const tasks=await readTasks(client,user.id);state=state?{...state,tasks}:{version:1,tasks}}catch(e:any){return Response.json({error:e?.message||'Erro ao carregar tarefas'},{status:500})}return Response.json({data:state,updated_at:data?.updated_at??null})}

export async function PUT(request:Request){const auth=await scopedClient(request);if(!('client'in auth)||!auth.client||!auth.user)return Response.json({error:auth.error},{status:auth.status});const client=auth.client;const user=auth.user;const body=await request.json().catch(()=>null);if(!body||typeof body.data!=='object'||Array.isArray(body.data))return Response.json({error:'Payload inválido'},{status:400});try{await syncAll(client,user.id,body.data)}catch(e:any){return Response.json({error:e?.message||'Erro ao sincronizar módulos'},{status:500})}const{error}=await client.from('mai_state').upsert({user_id:user.id,data:body.data,updated_at:new Date().toISOString()},{onConflict:'user_id'});if(error)return Response.json({error:error.message},{status:500});return Response.json({ok:true})}
